...
def zero_angle(ser):
    ...
    #     ser.write(b'\x01\xfd\x14\xff\x00\x00\x00\x00\x6B')
        send_pos = [0x01, 0xfd, 0x00, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x6B]
        send_pos[4:8] = struct.pack('>I', abs(current_angel))
        print("小於0 : ",send_pos)
        print("小於0 : ",bytes(send_pos))
        ser.write(bytes(send_pos))
    else:
        send_pos = [0x01,0xfd,0x10,0x0C,0x00,0x00,0x00,0x00,0x6B]
        send_pos[4:8] = struct.pack('>I', abs(current_angel))
        print("大於0 : ",send_pos)
        ser.write(bytes(send_pos))
...
def set_degress(ser,mode):
    ...
    if mode == 0:
        ...
        # 90度以上 情況
            ...
            send_pos[6:8] = struct.pack('>h', (current_angel - int(degree_list[mode]*44.4)))
            # ser.write(bytes(send_pos))
        ...
        if cudeg < 0:
            send_pos = [0x01, 0xfd, Backward, speed, 0x00, 0x00, 0x00, 0x00, 0x6B]
            send_pos[6:8] = struct.pack('>h', (current_angel * -1) - int(degree_list[mode] * 44.4))
        ...
        if cudeg < 0:
            ...
            ser.write(bytes(send_pos))
        # 90度以上 情況
        # elif (degree > current_degree // 5) > 90:
        #     send_pos = [0x01, 0xfd, 0x10, 0x0A, 0x00, 0x00, 0x00, 0x00, 0x6B]
        #     send_pos[6:8] = struct.pack('>h', (current_angel - (degree_list[mode] * 44)))
        #     ser.write(bytes(send_pos))
        #     print("移動到左邊-等於180 : ", send_pos)
        # 0~90度 情況
        else:
